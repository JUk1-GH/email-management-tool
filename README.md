...
`jemail` 是一个面向 Outlook / Microsoft 365 / Gmail 账号批量管理的 Web 工具。
...
- 邮箱账号、分组、邮件缓存存在浏览器 `IndexedDB`
- 后端新增 SQLite 用户库、普通资料云同步表，以及单独加密的完整凭据表
- Gmail 走官方 OAuth 2.0 + Gmail API，不走 Gmail IMAP XOAUTH2
...
- 用户级会话鉴权与数据隔离
- 普通资料云同步
- 完整账号资料：密码、辅助邮箱、2FA、client_id、refresh token 单独加密存储，登录后可直接拉取、查看或导出
- 新版 React 前端登录态、同步入口和移动端基础可用布局
